#!/bin/bash
#
# Goose CLI binary installation script
# ------------------------------------
#
...
CYN  "Goose CLI binary installation script"
echo "---------------------------------------"
echo ""
...
case "$PROCESSOR" in
    arm* | aarch* | ppc* )
        if [ "$OS_FLAVOUR" != Darwin ]; then
            ...
            echo "  => $(CYN https://github.com/metaplex-foundation/goose)"
            echo ""
            echo "to build Goose from the source code."
            exit 1
        fi
        ;;
...
RELEASE_URL="https://github.com/metaplex-foundation/goose/releases"
RELEASE="latest"
BIN="goose"
...
if [ $SIZE -eq 0 ]; then
    RED "Aborting: could not download Goose distribution"
    exit 1
fi
...
if [ ! "$(command -v $BIN)" = "" ]; then
    ...
    echo "Goose binary was found at:"
    echo "  => $(CYN $EXISTING)"
    echo ""
    ...
    if [ "$(command -v $BIN)" = "" ]; then
        ...
        if [ -f "$ENV_FILE" ]; then
            ...
            echo "  => adding '$TARGET' to 'PATH' variable to execute 'goose' from any directory."
...
# sanity check
if [ "$(command -v $BIN)" = "" ]; then
    # installation was completed, but Goose is not in the PATH
